...
const ALL_TAGS = ["all", "Distortion", "Typography", "Photoshop", "Glitch", "Duotone", "Print", "Film", "Neon", "Retro", "Surreal", "Bold"] as const;
...
function TemplateCard({ item }: { item: TemplateItem }) {
  ...
  const inner = (
    <div className="group relative isolate flex h-full flex-col overflow-hidden rounded-2xl bg-ink-800 ring-1 ring-inset ring-white/10 transition-all duration-300 hover:ring-primary-500/50 hover:shadow-[0_8px_40px_-12px_rgba(238,61,11,0.3)]">
      {/* Gradient bg */}
      <div aria-hidden className={cn("absolute inset-0 bg-linear-to-br opacity-80 transition-transform duration-700 group-hover:scale-110", item.accent)} />
      ...
      <div className="relative z-10 p-6">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <span className={cn("inline-flex items-center rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-wide ring-1 ring-inset backdrop-blur-sm", tagColor.bg, tagColor.text, tagColor.ring)}>
            {tag}
          </span>
          {!isReal && (
        ...
        <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-white/55">{description}</p>
        ...
        <div className="mt-4 flex items-center gap-3">
          {isReal ? (
            ...
            <span className="text-sm font-medium text-white/40">{t("templatesSection.comingSoon")}</span>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <main className="relative min-h-svh bg-ink-950">
      ...
      <div ref={sectionRef} className="relative px-5 pb-20 sm:px-10 lg:px-20">
        {/* Ambient glow */}
        <div aria-hidden className="pointer-events-none absolute -right-40 -top-40 h-[600px] w-[600px] rounded-full opacity-25 blur-[160px]" style={{ background: "radial-gradient(circle, #D73F18 0%, #300112 80%, transparent 100%)" }} />
        ...
        <div className="relative mx-auto max-w-[1440px]">
          ...
          {/* Stats row */}
          <div data-tpl-reveal className="mt-10 flex items-center justify-center gap-6 sm:gap-10">
            <div className="text-center">
              <p className="text-2xl font-bold text-white">11</p>
              <p className="mt-0.5 text-xs font-medium uppercase tracking-wide text-white/40">{t("templatesPage.statsTemplates")}</p>
            </div>
            <div className="h-6 w-px bg-white/10" />
            <div className="text-center">
              <p className="text-2xl font-bold text-white">7</p>
              <p className="mt-0.5 text-xs font-medium uppercase tracking-wide text-white/40">{t("templatesPage.statsCategories")}</p>
            </div>
            <div className="h-6 w-px bg-white/10" />
            <div className="text-center">
              <p className="text-2xl font-bold text-primary-400">Free</p>
              <p className="mt-0.5 text-xs font-medium uppercase tracking-wide text-white/40">{t("templatesPage.statsStarting")}</p>
            </div>
          </div>

          {/* Grid */}
          <div data-tpl-reveal className="mt-12 grid grid-cols-1 gap-5 sm:mt-14 sm:grid-cols-2 lg:grid-cols-3">
